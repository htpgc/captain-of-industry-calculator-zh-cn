...
async function generateLink() {
    ...
    try {
        ...
        if((err as Error)?.name !== 'AbortError') {
            showError(t('error.generateLink'), err);
        }
    } finally {
...
<template>
    <v-dialog v-model="dialog">
        <v-sheet>
            <v-toolbar>
                <v-toolbar-title>{{ t('link.generateTitle') }}</v-toolbar-title>
                <v-spacer />
                <v-btn
                    :icon="mdiClose"
                    @click="dialog = false"
