...
<template>
    <v-btn size="x-small" color="secondary" variant="outlined" @pointerdown.left.stop @click.stop>
        ...
        <v-menu v-model="menuOpened" density="compact" activator="parent" :close-on-content-click="false">
            <v-list>
                <v-list-item :title="t('factory.count')">
                    <input-number
                        class="count-number"
                ...
                <v-list-item class="d-flex justify-center">
                    <v-btn-toggle color="primary" multiple :model-value="itemOptions">
                        <tooltip-button :tooltip="t('factory.lock')" :icon="mdiLock" value="lock" @click="props.item.setLocked(!props.item.isLocked)" />
                        <tooltip-button
                            :tooltip="t('factory.objective.primary')"
                            :icon="mdiBullseyeArrow"
                            value="Primary"
                            @click="changeObjective(Objective.Primary)"
                        />
                        <tooltip-button
                            :tooltip="t('factory.objective.secondary')"
                            :icon="mdiBullseye"
                            value="Secondary"
                            @click="changeObjective(Objective.Secondary)"
                        />
                        <tooltip-button
                            :tooltip="t('factory.objective.low')"
                            :icon="mdiPriorityLow"
                            value="LowPriority"
                ...
                <v-list-item v-if="props.item.isUpgradable()" :title="t('factory.upgradeMode')" @click.stop="changeUpgradeMode();">
                    <template #prepend>
                        <v-switch
                            class="mr-5"
                            color="primary"
                ...
                <v-list-item
                    ...
                    :title="t('factory.deleteAllLinks')"
                    @click="props.item.deleteAllLinks(); menuOpened = false;"
                />
                <v-list-item
                    :prepend-icon="mdiDelete"
                    :title="t('factory.delete')"
