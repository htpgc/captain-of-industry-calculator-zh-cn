...
import {translateGameLabel} from '../i18n';
...
function createItemImpl(_item: GameItemSerialized, calculator: Calculator, order: number, game: string) {
    ...
    if(!item.exdata)
        item.exdata = {};
    //Added by Marvis: translate in-game data names, keep search working in both languages
    Object.defineProperty(item, 'label', {
        get: () => translateGameLabel(game, _item.label),
        enumerable: true,
        configurable: true,
    });
    Object.defineProperty(item, 'lowerLabel', {
        get: () => `${_item.label} ${translateGameLabel(game, _item.label)}`.toLowerCase(),
        enumerable: true,
        configurable: true,
    });
...
function createLogisticTransportImpl(logisticImpl: Readonly<GameLogisticImpl>, _transport: GameLogisticTransportSerialized, item: GameItem, game: string) {
    ...
    //Added by Marvis: translate in-game data names
    Object.defineProperty(transport, 'label', {
        get: () => (_transport.label === undefined ? undefined : translateGameLabel(game, _transport.label)),
        enumerable: true,
        configurable: true,
    });
...
function createLogisticImpl(_logistic: GameLogisticSerialized, parsedItems: ParsedItems, game: string) {
    const logistic: GameLogisticRaw = {
        ..._logistic,
        transport: [],
    };
    //Added by Marvis: translate in-game data names
    Object.defineProperty(logistic, 'label', {
        get: () => (_logistic.label === undefined ? undefined : translateGameLabel(game, _logistic.label)),
        enumerable: true,
        configurable: true,
    });
    ...
    for(const transport of _logistic.transport) {
        ...
        if(!item)
            continue;
        const transportImpl = createLogisticTransportImpl(logisticImpl, transport, item, game);
...
export function useGameDataParser(gameImplementation: GameImplementation): ParsedGameData {
    ...
    //Added by Marvis: game id used to pick the translation dictionary
    const game = gameData.description.name;
    ...
    gameData.items.forEach((value, index) => {
        const itemImpl = createItemImpl(value, calculator, index, game);
    ...
    const parsedLogistic = gameData.logistic?.map((logistic) => createLogisticImpl(logistic, parsedItems, game)) || [];
