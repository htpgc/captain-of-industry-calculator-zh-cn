...
//the product type name might be localized by the game language (e.g. Molten is exported as '热')
anyProductsByType.set('热', 'AnyMoltenProduct');
...
(async function() {
    ...
    //localized data support: register both the localized name and the english id as lookup keys,
    //because game exports may mix localized names (e.g. "水") with english ids (e.g. "Water")
    const productNamesToDefs = new Map<string, ProductDef>();
    const romanByNumber = ['I', 'II', 'III', 'IV', 'V'];
    for(const p of products.products) {
        const def: ProductDef = {...p, id: cleanProductId(p.id)};
        if(!productNamesToDefs.has(p.name)) {
            productNamesToDefs.set(p.name, def);
        }
        if(!productNamesToDefs.has(def.id)) {
            productNamesToDefs.set(def.id, def);
        }
        //"MaintenanceT3" <-> "Maintenance III"
        const maintenanceMatch = def.id.match(/^MaintenanceT(\d+)$/);
        if(maintenanceMatch) {
            const roman = romanByNumber[Number(maintenanceMatch[1]) - 1];
            if(roman && !productNamesToDefs.has(`Maintenance ${roman}`)) {
                productNamesToDefs.set(`Maintenance ${roman}`, def);
            }
        }
    }
...
async function prepareImages(productIdsToDefs: Map<string, ProductDef>, items: GameItemSerialized[]) {
    ...
    const resolveIcon = (iconPath: string | undefined, itemId: string) => {
        const name = getIconName(iconPath, itemId);
        return overrides[name] ?? overrides[itemId] ?? name;
    };
    ...
    for(const item of items) {
        if(item.recipe) {
            ...
            imagesPaths.push({
                id: item.name,
                icon: resolveIcon(def?.icon_path, item.name) + '.png',
                dir: buildingsDir,
            });
        } else {
    ...
    for(const image of imagesPaths) {
        ...
        if(!imagePath) {
            const altDir = image.dir === productsDir ? buildingsDir : productsDir;
            realImageName = altDir.map.get(image.icon.toLowerCase());
            if(realImageName) {
                imagePath = path.join(altDir.dir, realImageName);
                usedImages.add(realImageName);
            }
        }
        if(!imagePath) {
